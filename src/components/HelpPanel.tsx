...
export function HelpPanel() {
  ...
  return (
    <div className="space-y-10 animate-fade-in">
      ...
      <div className="bento-card rounded-2xl p-6 sm:p-8 relative overflow-hidden">
        ...
        <h2 className="text-2xl font-bold tracking-tight text-white mb-3">
          Guia do Usuário <span className="italic text-zinc-400 font-light">& Algoritmo de Inteligência</span>
        </h2>
        
        <p className="text-sm text-zinc-400 max-w-3xl leading-relaxed font-medium">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {steps.map((s, i) => (
            <div key={i} className="bento-card rounded-2xl p-6 flex flex-col justify-between group">
              <div>
                <div className="flex justify-between items-center mb-4">
                  <span className="text-3xl font-extrabold font-mono text-primary/10 group-hover:text-primary/20 transition-colors duration-300">
                    {s.num}
                  </span>
                  <span className="text-[9px] font-mono uppercase bg-white/5 px-2 py-0.5 rounded-full border border-white/5 text-zinc-400 font-semibold tracking-wider">
                    {s.badge}
                  </span>
                </div>
              ...
              <div className="pt-6 flex items-center gap-2 text-[10px] uppercase tracking-wider font-semibold text-primary mt-4 border-t border-white/5">
      ...
      <div className="bento-card rounded-2xl p-6 sm:p-8">
        <div className="inline-flex items-center gap-2 text-[10px] text-zinc-400 font-bold tracking-widest uppercase mb-4">
          <span className="w-1.5 h-1.5 rounded-full bg-primary shadow-[0_0_6px_#FF2A2A]"></span>
          Estrutura Heurística das Campanhas
        </div>
        
        <h3 className="text-xl font-bold text-white tracking-tight mb-2">
          Análise de Ranks <span className="italic text-zinc-400 font-light">& Classificação de Tráfego</span>
        </h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {classifications.map((c, i) => (
            <div key={i} className="p-5 bg-white/[0.02] border border-white/5 rounded-xl flex items-start gap-4 hover:border-white/10 transition-colors duration-200">
              <div className="w-10 h-10 flex-shrink-0 font-mono font-bold text-xs border border-primary/20 bg-primary/5 text-primary rounded-xl flex items-center justify-center shadow-[0_0_10px_rgba(255,42,42,0.1)]">
                [{c.rank}]
              </div>
      ...
      <div className="bento-card rounded-2xl p-6 sm:p-8 relative">
        ...
        <p className="text-xs text-zinc-400 mb-6 leading-relaxed max-w-xl font-medium">
        ...
        <div className="flex flex-wrap gap-2">
          {supportedPlatforms.map((p, i) => (
            <span
              key={i}
              className="text-xs font-semibold bg-white/[0.03] px-3.5 py-2 rounded-full border border-white/5 text-zinc-300 hover:border-primary/20 hover:text-white transition-all cursor-default select-none shadow-sm"
            >
              ✓ {p}
            </span>
          ))}
